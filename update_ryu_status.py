...
def getFlow(ids):															# Method to get flow each connected node 										
	for num in ids:															# for loop for connected node
		...
		if len(data_flow[str(num)]) != 0 :									# if have any data then update data in database
			if str(host_name) == "KKU":								# check host name to identify index 
               			host_desc = "KKU"
				index = 6
			...
			cur.execute("""UPDATE Online_Status SET src_mac=%s,dest_mac=%s,packet_count=%s WHERE host_name=%s""",(dl_dst,dl_dst2,packet_count,host_desc))
			db.commit()
...
for i in range (0,6):														# for loop to update data
	if db_status[i] == status[i]: 											# update when current status changed 
		duration = str(ts-db_start_time[i])[:-4]							# calculation duration time 
		cur.execute("""UPDATE Online_Status SET end_time=%s,src_mac=%s,dest_mac=%s,in_port=%s,out_port=%s,duration_time=%s,packet_count=%s WHERE host_name=%s""",(ts,src_mac[i],dest_mac[i],in_port[i],out_port[i],duration,packet[i],host[i]))
    	else:
		duration = ts-ts 													# initial duration time 
		cur.execute("""UPDATE Online_Status SET start_time=%s,end_time=%s,duration_time=%s,src_mac=%s,dest_mac=%s,in_port=%s,out_port=%s,packet_count=%s,statuss=%s WHERE host_name=%s""",(ts,ts,duration,src_mac[i],dest_mac[i],in_port[i],out_port[i],packet[i],status[i],host[i]))
		duration = str(ts-db_start_time[i])[:-4]							# calculation duration time 
		cur.execute("""INSERT INTO log_Online_status SET end_time=%s,src_mac=%s,dest_mac=%s,in_port=%s,out_port=%s,duration_time=%s,packet_count=%s,start_time=%s,host_name=%s,zone=%s,statuss=%s""",(ts,src_mac[i],dest_mac[i],in_port[i],out_port[i],duration,packet[i],db_start_time[i],row2[i],row3[i],status[i]))
